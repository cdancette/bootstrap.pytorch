...
class PlotlyAll():
    ...
    def __init__(self, exp_dir, fname='view.html'):
        super().__init__()
        ...
        for file in os.listdir(exp_dir):
            if file.endswith(".json"):
                self.log_names.append(file[:-5])
        # files = glob(os.path.join(exp_dir, '*.json'))
        # self.log_names = [name.strip('.json') for name in files]
    ...
    def generate(self):
        ...
        log_names = list(set(self.log_names)) # unique

        nb_keys = 0
        data_dict = {}
        items = []
        log_names_view_names = []  # (log_name, eval_epoch.loss)
        items = []  # log_name:eval_epoch.loss
        for log_name in log_names:
            path_json = os.path.join(self.exp_dir,
                                     '{}.json'.format(log_name))
            if os.path.isfile(path_json):
                ...
                with open(path_json, 'r') as handle:
                    data_json = json.load(handle)
                data_dict[log_name] = data_json
                nb_keys += len(data_dict[log_name])
                log_names_view_names.extend([(log_name, view_name) for view_name in data_dict[log_name].keys()])
                items.extend([log_name + ':' + item for item in data_dict[log_name].keys()])
            else:
                Logger()("Json log file '{}' not found in '{}'".format(log_name, path_json), log_level=Logger.WARNING)
        ...
        for figure_id, (log_name, view_name) in enumerate(log_names_view_names):
            print(log_name, view_name)
            # for figure_id, view_name in enumerate(data_dict[log_name]):
            if  not data_dict[log_name]:
                continue
            if 'batch' in view_name:
                continue
            views = [{  
                'view_interim': log_name + ':' + view_name,
                'log_name': log_name,
                'view_name': view_name,
                'split_name': view_name.split('.')[0],
                }]  # only one view per figure
            print(views[0]['view_interim'])
            ...
            for view in views:
                ...
                if view['split_name'] not in colors:
                    Logger()("Split '{}' not in colors '{}'".format(view['split_name'], list(colors.keys())), log_level=Logger.WARNING)
                    if 'eval_before' in view['split_name']:
                        color = 'rgb(31, 180, 80)'
                    elif 'eval_after' in view['split_name']:
